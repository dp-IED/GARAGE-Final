...
DEFAULT_LLM_BASE_URL = os.environ.get("LM_STUDIO_BASE_URL", "http://127.0.0.1:1234/v1")
DEFAULT_LLM_MODEL = os.environ.get("LM_STUDIO_MODEL", "granite-4.0-h-micro-GGUF")
HOST = "http://127.0.0.1:1234"
LLM_MODEL = DEFAULT_LLM_MODEL


def _normalize_server_root(url: str) -> str:
    """Normalize base URL to server root without /v1 suffix."""
...
def evaluate_gdn_kg_llm(
    ...
    global HOST, LLM_MODEL
    HOST = _normalize_server_root(base_url or DEFAULT_LLM_BASE_URL)
    LLM_MODEL = model_repo or DEFAULT_LLM_MODEL
    ...
    try:
        ...
        resp.raise_for_status()
        print(f"  ✓ Connected to LLM server at {HOST} (model: {LLM_MODEL})")
    except Exception as e:
        raise RuntimeError(
            f"Failed to connect to LLM server at {HOST}: {e}. "
            f"Please ensure an OpenAI-compatible HTTP server is running."
        )
...
def main():
    ...
    parser.add_argument(
        ...
        help="LLM model identifier on the OpenAI-compatible server",
    )
    parser.add_argument(
        "--base-url",
        type=str,
        default=DEFAULT_LLM_BASE_URL,
        help="OpenAI-compatible server base URL (with or without /v1)",
    )
    parser.add_argument(
        "--max-tokens",
        type=int,
