...
DEFAULT_LLM_BASE_URL = os.environ.get("LM_STUDIO_BASE_URL", "http://localhost:1234/v1")
DEFAULT_LLM_MODEL = os.environ.get("LM_STUDIO_MODEL", "granite-4.0-h-micro-GGUF")


def _normalize_base_url(url: str) -> str:
    """Ensure OpenAI-compatible endpoint root ends with /v1."""
...
def load_llm_model(
    ...
    print(f"Connecting to LLM server for model: {model_name}")
    inference = load_lm_studio_model(model_name=model_name, base_url=normalized_base)
...
def evaluate_llm_baseline(
    ...
    if model_repo is None:
        model_repo = DEFAULT_LLM_MODEL
    lm_base_url = _normalize_base_url(base_url or DEFAULT_LLM_BASE_URL)
    ...
    try:
        ...
        raise RuntimeError(
            f"Failed to connect to the LLM server: {e}. "
            f"Please ensure the OpenAI-compatible HTTP server is running."
        )
    ...
    # Create OpenAI client for structured output (OpenAI-compatible server)
    client = OpenAI(
...
def main():
    ...
    parser.add_argument(
        ...
        help="Model name on the OpenAI-compatible server",
    )
    parser.add_argument(
        "--base-url",
        type=str,
        default=DEFAULT_LLM_BASE_URL,
        help="Base URL for OpenAI-compatible chat server (default from LM_STUDIO_BASE_URL or localhost)",
    )
    parser.add_argument(
        "--model-path",
        type=str,
