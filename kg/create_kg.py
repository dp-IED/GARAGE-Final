...
def load_gdn_model(model_path: str, device: str = "cpu") -> Tuple[GDN, Dict[str, Any]]:
    ...
    calibrated = (
        checkpoint.get("calibrated_thresholds", {})
        if isinstance(checkpoint, dict)
        else {}
    )
    cal_sensor = float(calibrated.get("sensor", checkpoint.get("sensor_threshold", 0.3)))
    per_list = calibrated.get("per_sensor", [])
    per_np = np.asarray(per_list, dtype=np.float32) if per_list else None
    if per_np is not None and len(per_np) != len(sensor_names):
        per_np = None
    metadata["calibrated_sensor_threshold"] = cal_sensor
    metadata["calibrated_per_sensor_thresholds"] = per_np
...
class KnowledgeGraph:
    ...
    def __init__(
        ...
        self.distribution_thresholds = None  # Score percentiles for prompt metadata only
        self.X_windows = None  # Normalized windows
        self.X_windows_unnormalized = None  # Unnormalized windows
        self._calibrated_sensor_threshold = 0.5
        self._calibrated_per_sensor_thresholds: Optional[np.ndarray] = None

        # Initialize sensor nodes
        self._initialize_sensor_nodes()

    @property
    def calibrated_sensor_threshold(self) -> float:
        return float(self._calibrated_sensor_threshold)

    @property
    def calibrated_per_sensor_thresholds(self) -> Optional[np.ndarray]:
        return self._calibrated_per_sensor_thresholds

    def _calibrated_threshold_for_sensor(self, sensor_name: str) -> float:
        idx = self.sensor_to_idx.get(sensor_name, -1)
        if (
            self._calibrated_per_sensor_thresholds is not None
            and self._calibrated_per_sensor_thresholds.size > 0
            and 0 <= idx < int(self._calibrated_per_sensor_thresholds.size)
        ):
            return float(self._calibrated_per_sensor_thresholds[idx])
        return float(self._calibrated_sensor_threshold)
    ...
    def construct(
        ...
        Args:
            ...
            sensor_labels_true: Optional; passed through to percentile helper (unused)
            window_labels_true: Optional; passed through to percentile helper (unused)
            calibrated_sensor_threshold: Global checkpoint sensor score threshold for fault gating
            calibrated_per_sensor_thresholds: Optional length num_sensors vector of per-sensor thresholds
        ...
        self._calibrated_sensor_threshold = float(calibrated_sensor_threshold)
        if calibrated_per_sensor_thresholds is not None:
            arr = np.asarray(calibrated_per_sensor_thresholds, dtype=np.float64).reshape(
                -1
            )
            if arr.size == self.num_sensors:
                self._calibrated_per_sensor_thresholds = arr.astype(np.float32)
            else:
                self._calibrated_per_sensor_thresholds = None
        else:
            self._calibrated_per_sensor_thresholds = None

        # Percentile summaries from scores (LLM prompt metadata only; not used for fault labels).
    ...
    def _process_window(
        ...
        if self.distribution_thresholds:
            deviation_threshold = self.distribution_thresholds.get(
                "deviation_threshold", 0.3
            )
        else:
            deviation_threshold = 0.3

        # Add nodes (fault flags use checkpoint calibration, not score percentiles)
        for sensor_name, stats in window_stats.items():
            thr_c = self._calibrated_threshold_for_sensor(sensor_name)
            window_graph.add_node(
                ...
                is_faulty=bool(stats.anomaly_score > thr_c),
            )
    ...
    def _infer_semantic_edge(
        ...
        if edge_attrs.get("violates_domain_expectation", False) or edge_attrs.get(
            ...
            ti = self._calibrated_threshold_for_sensor(sensor_i)
            tj = self._calibrated_threshold_for_sensor(sensor_j)
            if stats_i.anomaly_score > ti or stats_j.anomaly_score > tj:
    ...
    def _build_temporal_edges(
        ...
        for sensor_name in self.sensor_names:
            ...
            thr = self._calibrated_threshold_for_sensor(sensor_name)
    ...
    def _track_anomaly_propagation(self, gdn_predictions: np.ndarray):
        ...
        for window_idx in range(num_windows):
            for sensor_idx, sensor_name in enumerate(self.sensor_names):
                thr = self._calibrated_threshold_for_sensor(sensor_name)
                if gdn_predictions[window_idx, sensor_idx] > thr:
        ...
        for root_sensor, root_window in first_occurrence_all.items():
            ...
            for window_idx in range(root_window + 1, num_windows):
                for sensor_idx, sensor_name in enumerate(self.sensor_names):
                    thr_a = self._calibrated_threshold_for_sensor(sensor_name)
                    if gdn_predictions[window_idx, sensor_idx] > thr_a:
    ...
    def _compute_distribution_thresholds(
        ...
        """
        Score percentiles over the test run for optional LLM prompt metadata.

        Not used for fault labels or graph gating; those use checkpoint calibration
        from construct(calibrated_sensor_threshold=..., ...).

        sensor_labels_true and window_labels_true are accepted for API
        compatibility with construct(); they are not used here.
        """
        _ = sensor_labels_true
        _ = window_labels_true

        preds = np.asarray(gdn_predictions, dtype=np.float64)
        if preds.size == 0:
            anomaly_threshold_global = 0.5
            anomaly_threshold_per_sensor = {
                name: anomaly_threshold_global for name in self.sensor_names
            }
        else:
            flat = preds.reshape(-1)
            anomaly_threshold_global = float(np.percentile(flat, 95))
    ...
    def get_window_kg(
        ...
        # Extract entities (fault flags: checkpoint calibration only)
        for sensor_name in self.sensor_names:
        ...
        # Extract relationships (anomaly involvement: same calibration as entities)
        anomalous_sensors = {
            sensor_name
            for sensor_name, stat in window_stats.items()
            if stat.anomaly_score > self._calibrated_threshold_for_sensor(sensor_name)
        }
        ...
        for prev_idx in range(
            ...
            if prev_idx in self.window_stats:
                ...
                for sensor_name, stat in prev_stats.items():
                    if (
                        stat.anomaly_score
                        > self._calibrated_threshold_for_sensor(sensor_name)
                    ):
...
def main():
    ...
    kg.construct(
        ...
        calibrated_sensor_threshold=float(
            metadata.get("calibrated_sensor_threshold", 0.5)
        ),
        calibrated_per_sensor_thresholds=metadata.get(
            "calibrated_per_sensor_thresholds"
        ),
    )
