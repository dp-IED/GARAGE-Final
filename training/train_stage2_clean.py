...
def _resolve_window_faulty_labels(
    npz_data: np.lib.npyio.NpzFile,
    sensor_labels: torch.Tensor,
    split_name: str,
) -> torch.Tensor:
    """
    Resolve binary per-window fault labels across schema variants.

    Preferred key: window_is_faulty (0/1). If unavailable or non-binary, derive from
    sensor labels (window is faulty if any sensor label is positive).
    """
    for key in ("window_is_faulty", "window_labels"):
        if key in npz_data:
            ...
            print(
                f"  [Compat:{split_name}] '{key}' is non-binary; deriving window labels from sensor_labels."
            )
            break

    return (sensor_labels.sum(dim=1) > 0).to(torch.long)
...
def main():
    ...
    train_window_labels = _resolve_window_faulty_labels(
        train_data, train_sensor_labels, "train"
    )
    ...
    val_window_labels = _resolve_window_faulty_labels(val_data, val_sensor_labels, "val")
