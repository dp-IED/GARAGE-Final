...
def _derive_forecast_targets_from_windows(
    windows: np.ndarray,
    drive_ids: np.ndarray,
    horizons: list[int],
) -> np.ndarray:
    """
    Build forecast targets from window arrays when explicit forecast targets are missing.

    Uses the last timestep from a future window within the same drive for each horizon.
    Falls back to the current window's last timestep when future context is unavailable.
    """
    n, _, d = windows.shape
    num_h = len(horizons)
    targets = np.zeros((n, num_h, d), dtype=np.float32)
    last_step = windows[:, -1, :].astype(np.float32)

    if drive_ids is None or len(drive_ids) != n:
        drive_ids = np.arange(n)
    else:
        drive_ids = np.asarray(drive_ids)

    for i in range(n):
        for h_idx, h in enumerate(horizons):
            j = i + int(h)
            if j < n and drive_ids[j] == drive_ids[i]:
                targets[i, h_idx] = windows[j, -1, :]
            else:
                targets[i, h_idx] = last_step[i]
...
def main():
    ...
    # Primary schema (data/create_shared_dataset.py output) uses clean windows + explicit targets.
    if (
        "clean_normalized_windows" in train_data
        and "forecast_targets" in train_data
        and "clean_normalized_windows" in val_data
        and "forecast_targets" in val_data
    ):
        train_windows = train_data["clean_normalized_windows"]
        val_windows = val_data["clean_normalized_windows"]
        train_targets = train_data["forecast_targets"]
        val_targets = val_data["forecast_targets"]
    else:
        # Compatibility schema (llm/evaluation/shared_dataset) ships only normalized_windows.
        # Derive multi-horizon targets from future windows so retraining works out-of-the-box.
        print(
            "  [Compat] Missing clean_normalized_windows/forecast_targets; "
            "deriving Stage-1 targets from normalized_windows."
        )
        if "normalized_windows" not in train_data or "normalized_windows" not in val_data:
            raise KeyError(
                "Dataset must provide either clean_normalized_windows+forecast_targets "
                "or normalized_windows for compatibility mode."
            )
        train_windows = train_data["normalized_windows"]
        val_windows = val_data["normalized_windows"]
        train_drive_ids_raw = (
            train_data["drive_ids"] if "drive_ids" in train_data else None
        )
        val_drive_ids_raw = val_data["drive_ids"] if "drive_ids" in val_data else None
        train_targets = _derive_forecast_targets_from_windows(
            train_windows, train_drive_ids_raw, FORECAST_HORIZONS
        )
        val_targets = _derive_forecast_targets_from_windows(
            val_windows, val_drive_ids_raw, FORECAST_HORIZONS
        )

    X_train          = torch.tensor(train_windows, dtype=torch.float32)
    y_train_forecast = torch.tensor(train_targets, dtype=torch.float32)
    drive_ids_train  = (
        train_data["drive_ids"] if "drive_ids" in train_data else np.arange(len(X_train))
    )
    X_val            = torch.tensor(val_windows, dtype=torch.float32)
    y_val_forecast   = torch.tensor(val_targets, dtype=torch.float32)
    drive_ids_val    = (
        val_data["drive_ids"] if "drive_ids" in val_data else np.arange(len(X_val))
    )
